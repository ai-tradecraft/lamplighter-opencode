...
.PHONY: help doctor install-deps setup ci lint qa run-chat-through-harness stop-chat-through-harness test-chat-through-harness test-container-adapter-operation test-real-backend print-real-backend-fingerprint check-uv
...
test-container-adapter-operation: check-uv ## Build the adapter image and verify the same adapter.operation envelope in local-container mode
	./scripts/test-container-adapter-operation.sh

run-chat-through-harness: ## Start the chat POC in three tmux panes and open the portal
	./scripts/run-chat-through-harness.sh

stop-chat-through-harness: ## Stop chat POC services started from this checkout
	./scripts/run-chat-through-harness.sh stop
